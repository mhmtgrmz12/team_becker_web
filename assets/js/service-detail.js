...
async function boot () {
  ...
  document.title = `${svc.name} — Civic Concierge`;
  const metaDesc = document.querySelector('meta[name="description"]');
  if (metaDesc) metaDesc.setAttribute("content", svc.blurb);

  // Hero
  const hero = document.getElementById("svc-hero");
  hero.innerHTML = "";
  hero.appendChild(heroBlock(svc));
  ...
  if (pbox) {
    pbox.innerHTML = "";
    pbox.appendChild(pricingCard(svc));
  }
...
function heroBlock (svc) {
  ...
  wrap.appendChild(create("div", { class: "lg:col-span-7" }, [
    create("div", { class: "inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6", style: "background:var(--surface-container-low);color:var(--secondary)", html: `<span class="msi" style="font-size:16px">${svc.icon}</span> Service detail` }),
    create("h1", { class: "text-4xl md:text-6xl font-bold tracking-tight leading-tight mb-5", style: "color:var(--primary)" }, svc.name),
    create("p", { class: "text-lg md:text-xl max-w-2xl leading-relaxed", style: "color:var(--secondary)" }, svc.blurb),
    create("div", { class: "mt-8 flex flex-wrap gap-3" }, [
      create("a", { href: `/request/?service=${svc.slug}`, class: "px-7 py-4 rounded-xl font-bold flex items-center gap-2", style: "background:var(--tertiary-fixed);color:var(--on-tertiary-fixed);box-shadow:var(--shadow-glow)", "data-track": "click_whatsapp_cta", "data-track-location": "service_hero", "data-track-serviceId": svc.id, html: '<span class="msi">chat</span> Start via WhatsApp' }),
      create("a", { href: "/pricing/", class: "px-7 py-4 rounded-xl font-semibold", style: "background:var(--surface-container-lowest);color:var(--primary);box-shadow:var(--shadow-soft)" }, "View pricing"),
    ]),
    create("div", { class: "mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm", style: "color:var(--secondary)" }, [
      create("span", { class: "flex items-center gap-1", html: '<span class="msi" style="font-size:18px;color:var(--on-tertiary-container)">schedule</span> ' + svc.turnaround }),
      create("span", { class: "flex items-center gap-1", html: '<span class="msi" style="font-size:18px;color:var(--on-tertiary-container)">payments</span> From €' + svc.serviceFeeFrom + ' + authority fees' }),
    ]),
  ]));
...
function docCard (d) {
  ...
  card.innerHTML = `
    ...
    <h3 class="text-lg font-bold mb-2" style="color:var(--primary)">${d.title}</h3>
    <p class="text-sm leading-relaxed mt-auto" style="color:var(--secondary)">${d.description}</p>
    ${d.formUrl ? `<a href="${d.formUrl}" class="mt-4 text-sm font-semibold" style="color:var(--on-tertiary-container)" data-track="download_form" data-track-formId="${d.id}">Download form →</a>` : ""}
  `;
  return card;
}
...
function extraCard (x) {
  ...
  card.innerHTML = `
    <div class="flex items-start gap-3">
      ...
      <div>
        <h4 class="font-bold mb-1" style="color:var(--primary)">${x.name}</h4>
        <p class="text-sm" style="color:var(--secondary)">${x.description || ""}</p>
        ${x.priceFrom ? `<p class="text-xs mt-2 font-bold" style="color:var(--on-tertiary-container)">+ from €${x.priceFrom}</p>` : ""}
      </div>
    </div>
  `;
  return card;
...
function pricingCard (svc) {
  ...
  wrap.innerHTML = `
    ...
    <h3 class="text-2xl font-bold mb-1" style="color:var(--primary)">${svc.name}</h3>
    <p class="mb-6" style="color:var(--secondary)">All-inclusive concierge handling.</p>
    <div class="flex items-baseline gap-2 mb-6">
      <span class="text-5xl font-black tracking-tighter" style="color:var(--primary)">€${svc.serviceFeeFrom}</span>
      <span class="font-medium" style="color:var(--secondary)">+ authority fees (${svc.authorityFeeHint})</span>
    </div>
    <ul class="space-y-3 mb-6 text-sm" style="color:var(--secondary)">
      <li class="flex items-center gap-2"><span class="msi" style="color:var(--tertiary-fixed-dim)">check_circle</span> Digital pre-verification</li>
      <li class="flex items-center gap-2"><span class="msi" style="color:var(--tertiary-fixed-dim)">check_circle</span> Priority at Kreis Steinfurt</li>
      <li class="flex items-center gap-2"><span class="msi" style="color:var(--tertiary-fixed-dim)">check_circle</span> WhatsApp status updates</li>
      <li class="flex items-center gap-2"><span class="msi" style="color:var(--tertiary-fixed-dim)">check_circle</span> Turnaround: ${svc.turnaround}</li>
    </ul>
    <a href="/request/?service=${svc.slug}" class="block w-full text-center px-6 py-4 rounded-xl font-bold" style="background:var(--primary);color:var(--on-primary)">Start this service →</a>
  `;
  return wrap;
}
...
function relatedCard (s) {
  ...
  a.innerHTML = `
    ...
    <h4 class="font-bold mb-1" style="color:var(--primary)">${s.name}</h4>
    <p class="text-sm" style="color:var(--secondary)">From €${s.serviceFeeFrom} · ${s.turnaround}</p>
  `;
  return a;
}
...
function faqItem (f) {
  ...
  d.innerHTML = `<summary class="flex justify-between items-center font-bold p-5" style="color:var(--primary)">${f.question}<span class="msi transition group-open:rotate-180" style="color:var(--secondary)">expand_more</span></summary><div class="px-5 pb-5" style="color:var(--secondary)">${f.answer}</div>`;
  return d;
}
